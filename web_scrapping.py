...
URL = 'httpskorean.visitkorea.or.krsearchsearch_list.dokeyword='+search
...
def page_work()
    ...
    result = driver.find_elements(By.CSS_SELECTOR,'#search_result .tita')
...
def file_export()
    ...
    df = pd.DataFrame({제목title_list, 내용contents_list}, index=None)
...
for page_no in range(1, page_cnt+1)    
    ...
    if page_no  page_cnt
        next_button = driver.find_element(By.CSS_SELECTOR, fa[id='{page_no+1}'])
